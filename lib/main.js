...
const {panelCommand, DockOptions} = require("./app/opquast-dock");
const webExtension = require("sdk/webextension");
const webExtBridge = require("./app/webExtBridge");

/**
 * listener to connectionless messages coming from the web extension
 * @param msg
 * @param sender
 * @param sendReply
 */
function webExtMessageListener(msg, sender, sendReply) {
    if (!("command" in msg)) {
        return;
    }

    switch(msg.command) {
        case "legacy-panel-command":
            panelCommand(msg.runTest, msg.canClose);
            break;
            /*sendReply({
...
exports.main = function(aOptions, aCallback) {
    ...
    webExtension.startup().then(({browser}) => {
        // let's wait after the connection with the webextension
        browser.runtime.onConnect.addListener(function (port) {
            webExtBridge.setPort(port);
            // now we can initialize the legacy extension
            sdkAddonStartup(aOptions, aCallback);
        });
        browser.runtime.onMessage.addListener(webExtMessageListener);
    });
};

function sdkAddonStartup(aOptions, aCallback) {
